...
datasets_dir = 'datasets_rttonly'

os.system('rm -rf %s'%datasets_dir)
os.system('mkdir %s'%datasets_dir)
...
file_list = []
with open('../drgnn/FILES_RTTONLY/test.csv', 'r') as f:
    r = csv.reader(f)
    for line in r:
        file = line[0]
        pdb = file.split('_')[0]
        if '_cm' not in file and '_ranair' not in file:
...
for foldnum in range(1,11):
    ...
    for split in ['train', 'valid']:
        ...
        file_list = []
        with open('../drgnn/FILES_RTTONLY/fold%d_%s.csv'%(foldnum,split), 'r') as f:
            r = csv.reader(f)
            for line in r:
                file = line[0]
                pdb = file.split('_')[0]
